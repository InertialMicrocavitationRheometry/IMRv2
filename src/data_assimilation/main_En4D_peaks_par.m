% parallel En4D_Var Ensemble Kalman filter for IMR solver
...
function [x,E2,P_post,params]= main_En4D_peaks_par(Model_params,t,yth,...
    R0_all,Req_all,tspan_all,peak_time_idx)
...
for j = 1:j_max
    ...
    while norm(dx1) > epsilon && jj <= max_iter
        ...
        disp(['ended iteration ',num2str(jj),' with norm(dx1) = ', ...
            num2str(norm(dx1)), ' and norm(dy2) = ' num2str(norm(dy2)), ...
            ' at ',num2str(timestep_time(jj)),' seconds for the ' ...
            num2str(exp_i) 'th experiments '])
