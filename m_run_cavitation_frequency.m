...
f_range = [0.5E6, 1E6, 2E6, 4E6, 8E6];
f_l_mu_max = zeros(length(f_range));
f_l_mu_avg = zeros(length(f_range));
R_max = zeros(length(f_range));
R_ave = zeros(length(f_range));
...
figure(1)
hold on;
plot(f_range*t0,f_l_mu_max,'or','MarkerFaceColor','r','MarkerSize',8);
plot(f_range*t0,R_max,'ob','MarkerFaceColor','b','MarkerSize',8);
